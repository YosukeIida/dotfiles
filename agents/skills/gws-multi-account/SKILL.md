...
metadata:
    vendor-repo: indentcorp/gws-multi-account
    vendor-path: skills/gws-multi-account/SKILL.md
    vendor-commit: e73dcbb12e581c51a259e0d5bf827b684faf997a
    vendor-extra-paths: skills/gws-multi-account/references/auth-login.md hooks/hook.js
---
...
5. Append the account to `accounts.json` (preserve existing entries). Use "$GWS_MULTI_ACCOUNT_NODE" (a nix-pinned Node.js binary) for cross-platform JSON merging — no `jq` dependency. There is no bare `node` on PATH in this environment by design; see the dotfiles nodeless policy.
   ```bash
   GWS_ACCOUNTS_JSON="$HOME/.config/gws/accounts.json" EMAIL=<email> DESC=<description> \
     "$GWS_MULTI_ACCOUNT_NODE" -e "const fs=require('fs'),p=process.env.GWS_ACCOUNTS_JSON;const d=fs.existsSync(p)?JSON.parse(fs.readFileSync(p,'utf8')):{};d[process.env.EMAIL]={description:process.env.DESC};fs.writeFileSync(p,JSON.stringify(d,null,2)+'\n');"
   ```
6. Verify:
   ```bash
